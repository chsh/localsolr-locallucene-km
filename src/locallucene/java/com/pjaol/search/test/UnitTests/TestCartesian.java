...
public class TestCartesian extends TestCase{
	...
	public void testRange() throws IOException, InvalidGeoException {
		...
		final double killometers = 6.0;
		
		// create a distance query
		final DistanceQuery dq = new DistanceQuery(lat, lng, killometers, latField, lngField, true);
		...
		CustomScoreQuery customScore = new CustomScoreQuery(tq,fsQuery){
			...
			public float customScore(int doc, float subQueryScore, float valSrcScore){
				...
				float score = new Float((killometers - distance) / killometers ).floatValue();
		...
		System.out.println("Results should be 4 "+ results);
		...
		for(int i =0 ; i < results; i++){
			...
			assertTrue((distance < killometers ));
		}
	}
